...
-- Set autoread.
map('n', '[Space]ar', function() Toggle_option("autoread") end)
-- Set spell check.
map('n', '[Space]p', function() Toggle_option("spell") end)
map('n', '[Space]w', function() Toggle_option("wrap") end)
...
map('n', '[Space]e', function() return vim.fn.expand("%") ~= '' and '<Cmd>edit %<CR>' or '' end, { expr = true })
...
-- The prefix key.
map('n', 's', '<Nop>')
map('n', 'sp', '<Cmd>vsplit<CR><Cmd>wincmd w<CR>')
...
map('n', '<C-f>',
  function()
    return math.max(vim.fn.winheight(0) - 2, 1) ..
        '<C-d>' .. (vim.fn.line('w$') >= vim.fn.line('$') and 'L' or 'M')
  end, { expr = true })
map('n', '<C-b>',
  function() return math.max(vim.fn.winheight(0) - 2, 1) .. '<C-u>' .. (vim.fn.line('w0') <= 1 and 'H' or 'M') end,
  { expr = true })
...
map('n', 'l', function() return vim.fn.foldclosed(vim.fn.line('.')) ~= -1 and 'zo0' or 'l' end, { expr = true })
-- If press l on fold, range fold open.
map('x', 'l', function() return vim.fn.foldclosed(vim.fn.line('.')) ~= -1 and 'zogv0' or 'l' end, { expr = true })
